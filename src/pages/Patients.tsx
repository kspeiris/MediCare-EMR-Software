      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editPatientId ? "Edit Patient Details" : "Add New Patient"}>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-[12px] font-medium text-slate-650 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">Cancel</button>
              ...
              <tr key={patient.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                ...
                <Td>
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleDelete(patient.id)} className="text-red-400 hover:text-red-650"><Trash2 size={13}/></button>
                  </div>
                </Td>
              </tr>
            ))}
